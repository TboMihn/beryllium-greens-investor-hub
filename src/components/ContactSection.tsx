...
const ContactSection = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Build mailto fallback (since no backend yet)
    const subject = encodeURIComponent(`Inquiry from ${form.name} — ${form.interest || "General"}`);
    const body = encodeURIComponent(
      `Name: ${form.name}\nEmail: ${form.email}\nPhone: ${form.phone || "N/A"}\nInterest: ${form.interest || "General"}\n\n${form.message}`
    );
    window.location.href = `mailto:info@berylliumgreens.com?subject=${subject}&body=${body}`;

    toast.success("Opening your email client — thank you for your interest!");
    setSending(false);
  };
    ...
    <SectionWrapper id="contact" className="bg-gradient-earth">
      <div className="text-center mb-14">
        ...
        <p className="font-body text-muted-foreground max-w-xl mx-auto">
          Interested in investing, partnering, or learning more? We'd love to hear from you.
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex gap-4 items-start">
            ...
            <div>
              ...
              <a href="mailto:info@berylliumgreens.com" className="font-body text-sm text-primary hover:underline">
          ...
          <div className="flex gap-4 items-start">
            ...
            <div>
              ...
              <p className="font-body text-sm text-muted-foreground">Reach out via the form to schedule a call.</p>
            </div>
          </div>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="lg:col-span-3 bg-background rounded-xl p-6 md:p-8 shadow-card border border-border space-y-5">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block font-body text-xs font-semibold text-foreground mb-1.5">Name *</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="block font-body text-xs font-semibold text-foreground mb-1.5">Email *</label>
              <input
                type="email"
                value={form.email}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block font-body text-xs font-semibold text-foreground mb-1.5">Phone</label>
              <input
                type="tel"
                value={form.phone}
            ...
            <div>
              <label className="block font-body text-xs font-semibold text-foreground mb-1.5">Interest</label>
              <select
                value={form.interest}
          ...
          <div>
            <label className="block font-body text-xs font-semibold text-foreground mb-1.5">Message *</label>
            <textarea
              value={form.message}
